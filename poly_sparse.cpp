...
template <typename T>    
Polynomial::Polynomial(const std::vector<T>& new_vector): coefficients() {
    size_t deg = 0;
    for (const T& new_coefficient : new_vector) {
        if (new_coefficient != T(0)) {
            coefficients.push_back(std::make_pair(deg, new_coefficient));
        }
        deg++;
    }
}
...
template <typename T>
Polynomial::Polynomial(const T& constant = T(0)) : coefficients() {
    if (constant != T(0)) {
        coefficients.push_back(std::make_pair(static_cast<size_t>(0), constant));
    }
}

template <class InputIt1, class InputIt2>
Polynomial::Polynomial(const InputIt1& in_it, const InputIt2& out_it): coefficients() {
    InputIt1 in_copy = in_it;
    size_t deg = 0;
    while (in_copy != out_it) {
        if (*in_copy != T(0)) {
            coefficients.push_back(std::make_pair(deg, *in_copy));
        }
        in_copy++;
        deg++;
    }
...
template <typename T>
typename std::vector<std::pair<size_t, T> >::const_iterator Polynomial::begin() const {
    return coefficients.begin();
}

template <typename T>
typename std::vector<std::pair<size_t, T> >::const_iterator Polynomial::end() const {
...
template <typename T>
T Polynomial::operator[] (size_t deg) const {
...
template <typename T>
const Polynomial<T>& Polynomial::operator+= (const Polynomial<T>& poly_right) {
    int right_deg = poly_right.Degree();
    int left_deg = Degree();
    std::vector<std::pair<size_t, T> > new_vector;
    int new_size = max(right_deg, left_deg);
    for (int i = 0; i <= new_size; i++) {
        T current_sum = (*this)[i] + poly_right[i];
        if (current_sum != T(0)) {
            new_vector.push_back(std::make_pair(
...
template <typename T>
const Polynomial<T>& Polynomial::operator+= (const T& scalar_right) {
...
template <typename T>
Polynomial<T> Polynomial::operator+ (const Polynomial<T>& poly_right) const {
...
template <typename T>
Polynomial<T> Polynomial::operator+ (const T& scalar_right) const {
    Polynomial<T> new_poly;
    new_poly = *this;
...
template <typename T>
Polynomial<T>& Polynomial::operator-= (const Polynomial<T>& poly_right) {
    int right_deg = poly_right.Degree();
    int left_deg = Degree();
    std::vector<std::pair<size_t, T> > new_vector;
    int new_size = max(right_deg, left_deg);
...
template <typename T>
Polynomial<T>& Polynomial::operator-= (const T& scalar_right) {
    Polynomial<T> helper(scalar_right);
    *this -= helper;
    return *this;
}
...
template <typename T>
const Polynomial<T> Polynomial::operator- (const Polynomial<T>& poly_right) const {
    Polynomial<T> new_poly;
    new_poly = *this;
    new_poly -= poly_right;
    return new_poly;
...
template <typename T>
const Polynomial<T> Polynomial::operator- (const T& scalar_right) const {
    Polynomial<T> new_poly;
    new_poly = *this;
    new_poly -= scalar_right;
    return new_poly;
...
template <typename T>
bool Polynomial::operator== (Polynomial<T>& other) const {
    return coefficients == other.coefficients;
}

template <typename T>
bool Polynomial::operator== (T scalar) const {
...
template <typename T>
T Polynomial::operator() (const T& coordinate) const {
    T value = T(0);
    T x = coordinate;
    int i = 0;
    if (i <= Degree()) {
...
Polynomial<T>& Polynomial::operator*= (const Polynomial<T>& poly_right) {
    ...
    std::vector<T> new_vector(new_degree + 1, T(0));
...
Polynomial<T>& Polynomial::operator*= (const T& scalar_right) {
    ...
    std::vector<T> new_vector(left_size, T());
...
Polynomial<T> Polynomial::operator/ (const Polynomial<T>& poly_right) const {
    ...
    Polynomial<T> rest;
    rest = *this;
    std::vector<T> result_vector((Degree() + 1), T(0));
...
Polynomial<T> Polynomial::operator% (const Polynomial<T>& poly_right) const {
    ...
    Polynomial<T> rest;
    rest = *this;
    // vector<T> result_vector(coefficients.size(), T(0));
    T new_coefficient = T(0);
    int new_degree;
    while (rest.Degree() >= poly_right.Degree()) {
        new_coefficient = rest[rest.Degree()] /
