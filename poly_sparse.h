...
public:
    Polynomial(const vector<T>& new_vector): coefficients() {
        size_t deg = 0;
    ...
    Polynomial(const T& constant = T(0)) : coefficients() {
        if (constant != T(0)) {
    ...
    Polynomial(const InputIt1& in_it, const InputIt2& out_it): coefficients() {
